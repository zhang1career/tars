...
Resources (GPIO pins, PWM channels, DAC channels) are listed in the pin map. At
boot, each entry gets a **default tenant** from CSV (`none|foc|system` only).
Runtime assignment lives in **RAM only** — it resets on power cycle / reset.

Assign a user-defined **tenant** label (alphanumeric + underscore, 1–15 chars)
...
Reserved names: `none` (unassigned), `foc`, `system` (pin-map defaults only;
cannot be used as user grant names).
...
Sections: `[periph]`, `[gpio]`, `[pwm]`, `[dac]`. Column `default_tenant`:
`none|foc|system`.
...
Grant a tenant from the shell before Lua uses a resource whose default tenant
is `none`.
