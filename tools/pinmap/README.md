...
[pwm]
channel,tim,chan,pin,af,alias
pwm0,tim1,1,pa8,AF1,
```

- **Pin names** (`pa8`, `pg13`, …) vary by board/package.
- **GPIO aliases** (`ld3`, `b1`) are optional board labels for shell lookup.
- **Resource tenants** are not in the CSV; assign at runtime (`mcu res grant`) and
  persist with `mcu res save`.
...
| `default_freq_hz` / `default_duty_pct` / `polarity` | Optional PWM design defaults (not tenant) |
