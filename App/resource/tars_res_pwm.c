                                          ...
                                          uint8_t duty_pct)
{
  ...
  follow_htim->Init.RepetitionCounter = master_htim->Init.RepetitionCounter;
...
static void pwm_link_hw_apply(TIM_HandleTypeDef *htim, int enable, int32_t offset_ticks)
{
  ...
  /* TIM3 runs center-aligned beside TIM1; phase is held by update-edge snap. */
...
static int pwm_link_snap_phase(void)
{
  ...
  st = pwm_link_wait_master_update(tim1);
  if (st != 0)
  {
    tim3->CR1 |= TIM_CR1_CEN;
    return st;
  ...
  cnt = (uint32_t)pwm_link_norm_offset((int32_t)tim1->CNT + s_link.offset_ticks, period);
...
int TarsResPwm_LinkGetStatus(char *out, uint32_t out_size)
{
  ...
  if (TarsResPwm_IsRunning(TARS_PWM_LINK_MASTER) != 0)
  {
    cnt_m = pwm_link_phase_ticks(tim1);
  }

  if ((follow_htim != NULL) && (TarsResPwm_IsRunning(TARS_PWM_LINK_FOLLOWER) != 0))
  {
    tim3 = follow_htim->Instance;
    cnt_f = pwm_link_phase_ticks(tim3);
  }
