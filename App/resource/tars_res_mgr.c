...
static void tim_domains_build(void)
{
  ...
  {
    TarsTenant_Copy((char *)s_tim_domains[(uint32_t)s_tim1_domain_idx].active_tenant,
...
static int res_default_locked(const tars_res_catalog_entry_t *cat)
{
  if (cat == NULL)
  {
    return 0;
  ...
  return TarsTenant_IsReservedDefault(cat->default_tenant);
}

static int res_grant_index(uint32_t idx, const char *tenant, const tars_res_catalog_entry_t *cat)
{
  if (idx >= s_slot_count)
  {
    return TARS_RES_ERR_SCOPE;
  ...
  if (res_default_locked(cat) != 0)
  {
    if ((TarsTenant_IsNone(tenant) != 0) ||
        (tenant_same(tenant, cat->default_tenant) == 0))
    {
...
void TarsResMgr_Init(void)
{
  ...
  for (i = 0U; i < count; i++)
  {
    TarsTenant_Copy(s_slots[i].tenant,
                    sizeof(s_slots[i].tenant),
                    cat[i].default_tenant);
...
int TarsResMgr_Grant(const char *id, const char *tenant)
{
  ...
  if (st == 0)
  {
    ...
    st = res_grant_index(idx, tenant, entry);
  }
  ...
  if ((st == 0) && (entry != NULL) && (entry->kind == TARS_RES_KIND_PWM))
  {
    ...
    if (TarsMcuPinmap_ResolvePwm(id, &pwm) == 0)
    {
      ...
      {
        if (res_default_locked(pin_cat) != 0)
        {
          st = TARS_RES_ERR_SYSTEM;
        }
        else if (!TarsTenant_IsNone(s_slots[pin_idx].active))
        {
...
int TarsResMgr_AcquirePwm(const char *channel)
{
      ...
      (TarsTenant_IsNone(s_slots[pin_idx].active)))
  {
    ...
    (void)TarsMcuPinmap_FindCatalog(pwm->pin_name, &pin_cat, &pin_idx);
    if ((pin_cat == NULL) || (res_default_locked(pin_cat) == 0))
    {
      TarsTenant_Copy(s_slots[pin_idx].tenant,
                      sizeof(s_slots[pin_idx].tenant),
