...
#define TARS_RES_PROFILE_VERSION    5U
#define TARS_PROF_FLAG_PWM_LINK     0x0001U
...
{
  uint32_t size = (uint32_t)sizeof(tars_prof_hdr_t) +
  ...
  if ((hdr->flags & TARS_PROF_FLAG_PWM_LINK) != 0U)
  {
    size += (uint32_t)sizeof(tars_prof_link_t);
  }
                               ...
                               uint32_t *size_out)
{
  ...
  if ((hdr->flags & TARS_PROF_FLAG_PWM_LINK) != 0U)
  {
    memcpy(out + size, link, sizeof(tars_prof_link_t));
    size += (uint32_t)sizeof(tars_prof_link_t);
  }
                                 ...
                                 const tars_prof_link_t *link)
{
  ...
  if ((hdr->flags & TARS_PROF_FLAG_PWM_LINK) != 0U)
  {
    crc = TarsCrc32((const uint8_t *)(const void *)link,
                    (uint32_t)sizeof(tars_prof_link_t),
                    crc);
  }
...
static int profile_collect_grants(tars_prof_grant_t *out, uint32_t *count_out)
{
  ...
  for (i = 0U; i < cat_count; i++)
  {
    ...
    if (TarsTenant_IsNone(runtime) != 0)
    {
      continue;
    }
...
static int profile_collect_pwm(tars_prof_pwm_t *out, uint32_t *count_out)
{
  ...
  for (i = 0U; i < pwm_count; i++)
  {
    ...
    if (TarsResPwm_GetPolarity(ch, &polarity_low) != 0)
    {
...
static int profile_parse_buf(const uint8_t *buf, uint32_t size)
{
  ...
  expected = profile_build_file_size(hdr);
  ...
  offset += hdr->tim_count * (uint32_t)sizeof(tars_prof_tim_t);

  if ((hdr->flags & TARS_PROF_FLAG_PWM_LINK) != 0U)
  {
    const tars_prof_link_t *link = (const tars_prof_link_t *)(const void *)(buf + offset);

    body_crc = profile_body_crc(hdr, grants, pwm, tim, link);
  ...
  body_crc = profile_body_crc(hdr, grants, pwm, tim, NULL);
  if (body_crc != hdr->body_crc32)
  {
    return TARS_RES_PROFILE_ERR_CRC;
  }
...
int TarsResProfile_Apply(void)
{
  ...
  {
    (void)TarsResPwm_LinkSet(1, s_staged.link.offset_ticks);
  }
...
int TarsResProfile_Save(void)
{
  ...
  memset(&s_staged.link, 0, sizeof(s_staged.link));
  ...
  hdr.flags = 0U;
  {
    ...
    if ((TarsResPwm_LinkGetConfig(&link_on, &link_off) == 0) && (link_on != 0))
    {
      hdr.flags |= TARS_PROF_FLAG_PWM_LINK;
      s_staged.link.offset_ticks = link_off;
    }
  }
  ...
  hdr.body_crc32 = profile_body_crc(&hdr,
                                    ...
                                    s_staged.pwm,
                                    s_staged.tim,
                                    ((hdr.flags & TARS_PROF_FLAG_PWM_LINK) != 0U)
                                        ? &s_staged.link
                                        : NULL);
...
int TarsResProfile_FormatStored(char *out, uint32_t out_size)
{
  ...
  if ((s_staged.hdr.flags & TARS_PROF_FLAG_PWM_LINK) != 0U)
  {
    ...
    (void)snprintf(out + (uint32_t)written,
                   out_size - (uint32_t)written,
                   "  link pwm0->pwm3 offset=%ld\r\n",
                   (long)s_staged.link.offset_ticks);
  }
