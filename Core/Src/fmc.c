...
#define SDRAM_REFRESH_COUNT     ((uint32_t)(((64UL * 36000000UL) / 8192UL) - 20UL))
...
static void SDRAM_Initialization_sequence(uint32_t refresh_count)
{
  ...
  __IO uint32_t tmpmrd = 0U;

  command.CommandMode = FMC_SDRAM_CMD_CLK_ENABLE;
  command.CommandTarget = FMC_SDRAM_CMD_TARGET_BANK2;
  command.AutoRefreshNumber = 1U;
  command.ModeRegisterDefinition = 0U;
  (void)HAL_SDRAM_SendCommand(&hsdram1, &command, SDRAM_TIMEOUT);
  ...
  (void)HAL_SDRAM_ProgramRefreshRate(&hsdram1, refresh_count);
}
